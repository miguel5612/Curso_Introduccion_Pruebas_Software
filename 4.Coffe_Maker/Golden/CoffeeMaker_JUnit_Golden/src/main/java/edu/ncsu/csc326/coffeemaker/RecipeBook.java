...
public class RecipeBook {
		...
		if (!exists) {
			for (int i = 0; i < recipeArray.length && !added; i++) {
				if (recipeArray[i] == null) {
					recipeArray[i] = r;
					added = true;
					System.out.println("Receta agregada en la posicion: " + i);
				}
			}
		}
		System.out.println("Estado de adicion de la receta: " + added);
		return added;
	}
	...
	public synchronized String deleteRecipe(int recipeToDelete) {
		if (recipeArray[recipeToDelete] != null) {
			...
			System.out.println("La receta ahora es: " + recipeArray[recipeToDelete]);
			return recipeName;
		} else {
			return null;
		}
