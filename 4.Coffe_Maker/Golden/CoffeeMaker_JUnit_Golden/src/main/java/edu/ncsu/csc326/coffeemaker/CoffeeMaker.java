...
public class CoffeeMaker {
	...
	public synchronized int makeCoffee(int recipeToPurchase, int amtPaid) {
		int change = 0;
		System.out.println("Entre a makecoffeeeee!!!!!!");
		if (getRecipes()[recipeToPurchase] == null) {
			change = amtPaid;
			System.out.println("El sistema no ha encontrado la receta");
			...
			if (inventory.useIngredients(getRecipes()[recipeToPurchase])) {
				...
				System.out.println("El precio de la receta es de " + getRecipes()[recipeToPurchase].getPrice());
				System.out.println("Dinero tienes: " + amtPaid);
				System.out.println("Tu cambio es de: " + change);
			} else {
				System.out.println("La cafetera se ha quedado sin ingredientes");
				change = amtPaid;
			}
		} else {
			System.out.println("El sistema encontro que te falta dinero");
			change = amtPaid;
		}
